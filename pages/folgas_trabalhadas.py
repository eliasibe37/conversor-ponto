...
st.title("📊 Folgas Trabalhadas")
st.markdown("Análise da solicitação das folgas trabalhadas.")

# Organização dos uploads na tela
col1, col2, col3 = st.columns(3)

with col1:
    st.subheader("1. Escala ")
    arquivo_escala = st.file_uploader("Upload da Escala (Excel)", type=["xlsx"])

with col2:
    st.subheader("2. Frequência / Movimentação")
    arquivo_movimento = st.file_uploader("Upload do Movimento (Excel)", type=["xlsx"])

with col3:
    st.subheader("3. Folgas Trabalhadas Solicitadas")
